...
function addServer(discordID,voiceID){
    ...
    db.query(q).then( res => {
        //db.query("SELECT SUM(total_cost) FROM swear_log WHERE guild_id = '"+discordID+"' AND vc_id = '"+voiceID+"';").then(res => jarTotal = res.row['total_cost']).catch(e => console.log(e.stack))
